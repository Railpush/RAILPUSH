...
ensure_railpush_env() {
  ...
  CORS_ALLOWED_ORIGINS="${CORS_ALLOWED_ORIGINS:-https://${DEPLOY_DOMAIN},https://www.${DEPLOY_DOMAIN},http://localhost:5173,http://127.0.0.1:5173}"
  DOCKER_NETWORK="${DOCKER_NETWORK:-railpush}"
  ENCRYPTION_KEY="${ENCRYPTION_KEY:-$(generate_secret 16)}"

  GITHUB_CLIENT_ID="${GITHUB_CLIENT_ID:-}"
  GITHUB_CLIENT_SECRET="${GITHUB_CLIENT_SECRET:-}"
  GITHUB_CALLBACK_URL="${GITHUB_CALLBACK_URL:-https://railpush.com/api/v1/auth/github/callback}"
