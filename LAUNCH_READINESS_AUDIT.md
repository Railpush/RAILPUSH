...
1) Transactional email is not fully "production enabled"
   - Code support exists (outbox + SMTP sender + templates), but you still need:
     - A provider decision (SMTP creds).
     - DNS deliverability (SPF/DKIM/DMARC).
     - K8s secrets wired (`EMAIL_PROVIDER=smtp`, `SMTP_*`).
   - Without this, users will not receive account/deploy notifications.

2) External paging is not enabled
   - We ingest Alertmanager alerts and show incidents, but "page humans" still needs:
     - Alertmanager receivers for Slack/email (or RailPush -> Slack/email on incident triggers).
   - Without this, you have no guaranteed on-call notification path.

3) Backups/restore drills are not proven
...
4) k3s HA operational sharp edge: kubectl logs/exec depend on which server you target
...
1) Configure transactional email end-to-end (provider + DNS + K8s secrets) and send a test welcome + deploy email.
2) Enable paging (Alertmanager -> Slack/email) and run a simulated alert to confirm delivery.
3) Run a CNPG restore drill and document results (timestamps, duration, pitfalls).
4) Fix the k3s multi-server log/exec behavior (either standardize ops access via one API endpoint, or re-architect HA/LB so logs/exec are reliable).
