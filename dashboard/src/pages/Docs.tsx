  ...
  "docs_url": "https://docs.railpush.com/api/errors#SERVICE_NOT_FOUND"
              ...
              {[
                ...
                { group: 'Logs & Metrics', endpoints: [
                  { method: 'GET', path: '/services/:id/logs', desc: 'Query service logs (supports type,search,regex,since,until,level,filter for structured fields)' },
                  { method: 'GET', path: '/services/:id/log-drains', desc: 'List configured external log forwarding drains for a service' },
                  { method: 'POST', path: '/services/:id/log-drains', desc: 'Create a log drain (destination, config, filters, and delivery controls)' },
                  { method: 'DELETE', path: '/services/:id/log-drains/:drainId', desc: 'Delete a configured log drain' },
                  { method: 'GET', path: '/services/:id/log-drains/:drainId/stats', desc: 'Get log drain delivery stats (sent, failed, lag, last error)' },
                  { method: 'POST', path: '/services/:id/log-drains/:drainId/test', desc: 'Send a synthetic test log entry through a drain' },
                  { method: 'GET', path: '/services/:id/log-alerts', desc: 'List log-based alert rules for a service' },
                  { method: 'POST', path: '/services/:id/log-alerts', desc: 'Create a log-based alert rule (filter/pattern + threshold/window + channels/cooldown)' },
                ...
                { group: 'Autoscaling & Jobs', endpoints: [
                  ...
                  { method: 'POST', path: '/services/:id/shell', desc: 'Create persistent shell session for a service container' },
                  { method: 'POST', path: '/shell/:sessionId/exec', desc: 'Execute command inside an existing shell session' },
                  { method: 'DELETE', path: '/shell/:sessionId', desc: 'Close shell session' },
                  { method: 'GET', path: '/services/:id/fs', desc: 'List files/directories in service container (read-only)' },
                  { method: 'GET', path: '/services/:id/fs/read', desc: 'Read file content from service container (read-only, size-limited)' },
                  { method: 'GET', path: '/services/:id/fs/search', desc: 'Search files in service container by glob pattern' },
                ...
                { group: 'Databases (PostgreSQL)', endpoints: [
                  ...
                  { method: 'GET', path: '/databases/:id/recovery-window', desc: 'Get available point-in-time recovery window' },
                  ...
                  { method: 'POST', path: '/databases/:id/restore', desc: 'Restore a soft-deleted database, or perform time-targeted restore with target_time (new database by default, optional in-place)' },
                ...
                { group: 'Workspace & Billing', endpoints: [
                  ...
                  { method: 'GET', path: '/workspaces/:id/compliance', desc: 'Get workspace compliance guardrails (phase 1)' },
                  { method: 'PUT', path: '/workspaces/:id/compliance', desc: 'Set workspace compliance guardrails (phase 1)' },
                  { method: 'GET', path: '/workspaces/:id/compliance/report', desc: 'Generate SOC2/HIPAA/GDPR policy readiness report (phase 1)' },
                ...
                <table className="w-full text-sm border-collapse">
                  ...
                  <tbody className="text-content-secondary">
                    {[
                      ...
                      ['Databases', 'list (plan/status/version/name/query filters + limit/cursor pagination), create, get (redacted), reveal credentials, rotate password, query_database (read-only by default, optional write mode), get/set retention policy, get recovery window, update, delete (token-confirmed soft delete with optional linked-service confirmation), restore (soft-delete + point-in-time modes), list restore jobs, backup, list backups (limit/cursor pagination), replicas, create replica, promote replica, enable HA'],
