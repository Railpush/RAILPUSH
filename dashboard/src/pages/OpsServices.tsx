...
export function OpsServicesPage() {
  ...
  const [rows, setRows] = useState<OpsServiceItem[]>([]);
  ...
  const load = () => {
    ...
    setError(null);
    ops
      .listServices({ status: status === 'all' ? '' : status, query: q, limit: 200 })
      .then(setRows)
      .catch((e) => {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h1 className="text-3xl font-semibold text-content-primary mt-1">Services</h1>
          <p className="text-sm text-content-secondary mt-1">Global service inventory.</p>
        </div>
      ...
      <Card className="p-0 overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 border-b border-border-default/60">
          ...
          <div className="relative w-full md:w-[360px]">
            ...
            <input
              type="text"
              placeholder="Search name, workspace, repo..."
              value={query}
        ...
        {forbidden ? (
          ...
          <div className="p-10 text-center text-sm text-content-tertiary">No services.</div>
        ) : (
          <div className="overflow-hidden">
            ...
            {rows.map((s) => (
              <div key={s.id} className="grid grid-cols-12 px-4 py-3 border-b border-border-subtle">
                <div className="col-span-4 min-w-0">
                  <button
                    onClick={() => navigate(`/ops/services/${encodeURIComponent(s.id)}/logs`)}
                  ...
                  <div className="text-xs text-content-tertiary truncate">{s.type} · {s.runtime} · {s.subdomain} · <span className="font-mono">{s.id.slice(0, 8)}</span></div>
                </div>
                <div className="col-span-3 min-w-0">
                  <div className="text-sm text-content-secondary truncate">{s.workspace_name || s.workspace_id.slice(0, 8)}</div>
                </div>
                <div className="col-span-3 min-w-0">
