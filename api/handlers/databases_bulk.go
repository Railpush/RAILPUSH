...
func (h *DatabaseHandler) BulkUpdateDatabases(w http.ResponseWriter, r *http.Request) {
	var req struct {
		IDs             []string               `json:"ids"`
	...
	ids := normalizeBulkIDs(req.IDs, req.DatabaseIDs)
	if len(ids) == 0 {
	...
	runPreflight := func(id string) (bulkResultItem, bool) {
		...
		code, body := runSubrequest(h.UpdateDatabase, subReq)
		...
		item.Status = "failed"
		item.Error = parseErrorBody(body, "failed to validate database update")
		return item, false
	}
	...
	for _, id := range ids {
		...
		if code >= 200 && code < 300 {
			...
			item.Status = "failed"
			item.Error = parseErrorBody(body, "failed to update database")
		}
		results = append(results, item)
	}
