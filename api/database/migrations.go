...
$$ LANGUAGE plpgsql IMMUTABLE`,
	...
	`ALTER TABLE workspaces ADD COLUMN IF NOT EXISTS compliance_data_residency VARCHAR(32) NOT NULL DEFAULT 'global'`,
	`ALTER TABLE workspaces ADD COLUMN IF NOT EXISTS compliance_require_encryption_at_rest BOOLEAN NOT NULL DEFAULT FALSE`,
	`ALTER TABLE workspaces ADD COLUMN IF NOT EXISTS compliance_require_mfa_for_destructive BOOLEAN NOT NULL DEFAULT FALSE`,
	`ALTER TABLE workspaces ADD COLUMN IF NOT EXISTS compliance_session_timeout_minutes INT NOT NULL DEFAULT 30`,
	`ALTER TABLE workspaces ADD COLUMN IF NOT EXISTS compliance_ip_allowlist_required BOOLEAN NOT NULL DEFAULT FALSE`,
	...
	// Persistent service shell sessions (container exec with stateful cwd/env).
	`CREATE TABLE IF NOT EXISTS service_shell_sessions (
		id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
		service_id UUID NOT NULL REFERENCES services(id) ON DELETE CASCADE,
		workspace_id UUID NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
		created_by UUID REFERENCES users(id) ON DELETE SET NULL,
		cwd TEXT NOT NULL DEFAULT '/',
		base_env_json JSONB NOT NULL DEFAULT '{}'::jsonb,
		current_env_json JSONB NOT NULL DEFAULT '{}'::jsonb,
	...
	`CREATE TABLE IF NOT EXISTS database_restore_jobs (
		id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
		source_database_id UUID NOT NULL REFERENCES managed_databases(id) ON DELETE CASCADE,
		target_database_id UUID REFERENCES managed_databases(id) ON DELETE SET NULL,
		workspace_id UUID NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
		backup_id UUID REFERENCES backups(id) ON DELETE SET NULL,
		target_time TIMESTAMPTZ NOT NULL,
		effective_restore_point TIMESTAMPTZ,
		restore_to TEXT NOT NULL DEFAULT 'new_database',
	...
	`CREATE INDEX IF NOT EXISTS idx_database_restore_jobs_source_created ON database_restore_jobs(source_database_id, created_at DESC)`,
	`CREATE INDEX IF NOT EXISTS idx_database_restore_jobs_target_created ON database_restore_jobs(target_database_id, created_at DESC)`,
	`CREATE INDEX IF NOT EXISTS idx_database_restore_jobs_workspace_created ON database_restore_jobs(workspace_id, created_at DESC)`,
	`ALTER TABLE database_restore_jobs ADD COLUMN IF NOT EXISTS source_database_id UUID`,
	`ALTER TABLE database_restore_jobs ADD COLUMN IF NOT EXISTS target_database_id UUID`,
	`ALTER TABLE database_restore_jobs ADD COLUMN IF NOT EXISTS workspace_id UUID`,
	`ALTER TABLE database_restore_jobs ADD COLUMN IF NOT EXISTS backup_id UUID`,
	`ALTER TABLE database_restore_jobs ADD COLUMN IF NOT EXISTS target_time TIMESTAMPTZ`,
	`ALTER TABLE database_restore_jobs ADD COLUMN IF NOT EXISTS effective_restore_point TIMESTAMPTZ`,
	`ALTER TABLE database_restore_jobs ADD COLUMN IF NOT EXISTS restore_to TEXT NOT NULL DEFAULT 'new_database'`,
	`ALTER TABLE database_restore_jobs ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'queued'`,
	`ALTER TABLE database_restore_jobs ADD COLUMN IF NOT EXISTS error TEXT NOT NULL DEFAULT ''`,
	`ALTER TABLE database_restore_jobs ADD COLUMN IF NOT EXISTS requested_by UUID`,
	`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='database_restore_jobs_source_database_id_fkey') THEN ALTER TABLE database_restore_jobs ADD CONSTRAINT database_restore_jobs_source_database_id_fkey FOREIGN KEY (source_database_id) REFERENCES managed_databases(id) ON DELETE CASCADE; END IF; END $$`,
	`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='database_restore_jobs_target_database_id_fkey') THEN ALTER TABLE database_restore_jobs ADD CONSTRAINT database_restore_jobs_target_database_id_fkey FOREIGN KEY (target_database_id) REFERENCES managed_databases(id) ON DELETE SET NULL; END IF; END $$`,
	`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='database_restore_jobs_workspace_id_fkey') THEN ALTER TABLE database_restore_jobs ADD CONSTRAINT database_restore_jobs_workspace_id_fkey FOREIGN KEY (workspace_id) REFERENCES workspaces(id) ON DELETE CASCADE; END IF; END $$`,
	`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='database_restore_jobs_backup_id_fkey') THEN ALTER TABLE database_restore_jobs ADD CONSTRAINT database_restore_jobs_backup_id_fkey FOREIGN KEY (backup_id) REFERENCES backups(id) ON DELETE SET NULL; END IF; END $$`,
	`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='database_restore_jobs_requested_by_fkey') THEN ALTER TABLE database_restore_jobs ADD CONSTRAINT database_restore_jobs_requested_by_fkey FOREIGN KEY (requested_by) REFERENCES users(id) ON DELETE SET NULL; END IF; END $$`,
	`ALTER TABLE database_restore_jobs ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()`,
	`ALTER TABLE database_restore_jobs ADD COLUMN IF NOT EXISTS started_at TIMESTAMPTZ`,
	`ALTER TABLE database_restore_jobs ADD COLUMN IF NOT EXISTS finished_at TIMESTAMPTZ`,
	`UPDATE database_restore_jobs SET restore_to='new_database' WHERE COALESCE(restore_to,'') = ''`,
	`UPDATE database_restore_jobs SET status='queued' WHERE COALESCE(status,'') = ''`,
	`UPDATE database_restore_jobs SET error='' WHERE error IS NULL`,

	// Service log alerting (pattern/filter-based alert rules evaluated against runtime logs).
	`CREATE TABLE IF NOT EXISTS service_log_alerts (
		id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
		service_id UUID NOT NULL REFERENCES services(id) ON DELETE CASCADE,
		workspace_id UUID NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
		name TEXT NOT NULL,
		...
		channels TEXT[] NOT NULL DEFAULT '{incident}'::text[],
	...
	`CREATE INDEX IF NOT EXISTS idx_service_log_alerts_service ON service_log_alerts(service_id, created_at DESC)`,
	`CREATE INDEX IF NOT EXISTS idx_service_log_alerts_enabled ON service_log_alerts(enabled, updated_at DESC)`,
	`ALTER TABLE service_log_alerts ADD COLUMN IF NOT EXISTS workspace_id UUID`,
	`UPDATE service_log_alerts a
	    SET workspace_id = s.workspace_id
	   FROM services s
	  WHERE a.service_id = s.id
	    AND a.workspace_id IS NULL`,
	`ALTER TABLE service_log_alerts ALTER COLUMN workspace_id SET NOT NULL`,
	`ALTER TABLE service_log_alerts ADD COLUMN IF NOT EXISTS filter_query TEXT NOT NULL DEFAULT ''`,
	`ALTER TABLE service_log_alerts ADD COLUMN IF NOT EXISTS pattern TEXT NOT NULL DEFAULT ''`,
	`ALTER TABLE service_log_alerts ADD COLUMN IF NOT EXISTS threshold INT NOT NULL DEFAULT 1`,
	`ALTER TABLE service_log_alerts ADD COLUMN IF NOT EXISTS window_seconds INT NOT NULL DEFAULT 300`,
	`ALTER TABLE service_log_alerts ADD COLUMN IF NOT EXISTS comparison TEXT NOT NULL DEFAULT 'greater_than'`,
	`ALTER TABLE service_log_alerts ADD COLUMN IF NOT EXISTS cooldown_seconds INT NOT NULL DEFAULT 900`,
	`ALTER TABLE service_log_alerts ADD COLUMN IF NOT EXISTS channels TEXT[] NOT NULL DEFAULT '{incident}'::text[]`,
	`ALTER TABLE service_log_alerts ADD COLUMN IF NOT EXISTS webhook_url TEXT NOT NULL DEFAULT ''`,
	`ALTER TABLE service_log_alerts ADD COLUMN IF NOT EXISTS priority TEXT NOT NULL DEFAULT 'normal'`,
	`ALTER TABLE service_log_alerts ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'ok'`,
	`ALTER TABLE service_log_alerts ADD COLUMN IF NOT EXISTS last_match_count INT NOT NULL DEFAULT 0`,
	`ALTER TABLE service_log_alerts ADD COLUMN IF NOT EXISTS last_evaluated_at TIMESTAMPTZ`,
	`ALTER TABLE service_log_alerts ADD COLUMN IF NOT EXISTS last_triggered_at TIMESTAMPTZ`,
	`ALTER TABLE service_log_alerts ADD COLUMN IF NOT EXISTS last_resolved_at TIMESTAMPTZ`,
	`ALTER TABLE service_log_alerts ADD COLUMN IF NOT EXISTS last_error TEXT NOT NULL DEFAULT ''`,
	`ALTER TABLE service_log_alerts ADD COLUMN IF NOT EXISTS created_by UUID`,
	...
	// Service log drains (forward logs to external observability systems).
	`CREATE TABLE IF NOT EXISTS service_log_drains (
		id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
		service_id UUID NOT NULL REFERENCES services(id) ON DELETE CASCADE,
		workspace_id UUID NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
		name TEXT NOT NULL,
		...
		filter_log_types TEXT[] NOT NULL DEFAULT '{app}'::text[],
		filter_min_level TEXT NOT NULL DEFAULT 'info',
	...
	`CREATE INDEX IF NOT EXISTS idx_service_log_drains_service ON service_log_drains(service_id, created_at DESC)`,
	`CREATE INDEX IF NOT EXISTS idx_service_log_drains_enabled ON service_log_drains(enabled, updated_at ASC)`,
	`ALTER TABLE service_log_drains ADD COLUMN IF NOT EXISTS workspace_id UUID`,
	`UPDATE service_log_drains d
	    SET workspace_id = s.workspace_id
	   FROM services s
	  WHERE d.service_id = s.id
	    AND d.workspace_id IS NULL`,
	`ALTER TABLE service_log_drains ALTER COLUMN workspace_id SET NOT NULL`,
	`ALTER TABLE service_log_drains ADD COLUMN IF NOT EXISTS destination TEXT NOT NULL DEFAULT 'webhook'`,
	`ALTER TABLE service_log_drains ADD COLUMN IF NOT EXISTS config_encrypted TEXT NOT NULL DEFAULT ''`,
	`ALTER TABLE service_log_drains ADD COLUMN IF NOT EXISTS filter_log_types TEXT[] NOT NULL DEFAULT '{app}'::text[]`,
	`ALTER TABLE service_log_drains ADD COLUMN IF NOT EXISTS filter_min_level TEXT NOT NULL DEFAULT 'info'`,
	`ALTER TABLE service_log_drains ADD COLUMN IF NOT EXISTS include_patterns TEXT[] NOT NULL DEFAULT '{}'::text[]`,
	`ALTER TABLE service_log_drains ADD COLUMN IF NOT EXISTS exclude_patterns TEXT[] NOT NULL DEFAULT '{}'::text[]`,
	`ALTER TABLE service_log_drains ADD COLUMN IF NOT EXISTS enabled BOOLEAN NOT NULL DEFAULT TRUE`,
	`ALTER TABLE service_log_drains ADD COLUMN IF NOT EXISTS sent_count BIGINT NOT NULL DEFAULT 0`,
	`ALTER TABLE service_log_drains ADD COLUMN IF NOT EXISTS failed_count BIGINT NOT NULL DEFAULT 0`,
	`ALTER TABLE service_log_drains ADD COLUMN IF NOT EXISTS last_error TEXT NOT NULL DEFAULT ''`,
	`ALTER TABLE service_log_drains ADD COLUMN IF NOT EXISTS last_delivery_at TIMESTAMPTZ`,
	`ALTER TABLE service_log_drains ADD COLUMN IF NOT EXISTS last_cursor_at TIMESTAMPTZ`,
	`ALTER TABLE service_log_drains ADD COLUMN IF NOT EXISTS last_test_at TIMESTAMPTZ`,
	`ALTER TABLE service_log_drains ADD COLUMN IF NOT EXISTS created_by UUID`,
	`DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='service_log_drains_created_by_fkey') THEN ALTER TABLE service_log_drains ADD CONSTRAINT service_log_drains_created_by_fkey FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE SET NULL; END IF; END $$`,
	`ALTER TABLE service_log_drains ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()`,
	`ALTER TABLE service_log_drains ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()`,
}
